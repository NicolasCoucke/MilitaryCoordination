...
# now just add the EEG data et voila
...
# now make a dataframe for sync and one for desync where only the data of the leader is stored
df_EEG_avg <- df_EEG %>% group_by(group, pair, participant, condition) %>%  # Group by the specified columns
summarise_at(c('alpha_frontal', 'alpha_parietal_motor', 'alpha_parietal_parietal', 'alpha_motor_motor'), mean, na.rm = TRUE)

df_EEG_sync <- df_EEG_avg %>% filter(condition %in% c("Synchronous/LeaderFollower", "Synchronous/FollowerLeader"))
filtered_EEG_sync <- df_EEG_sync  %>%
  filter((condition == "Synchronous/LeaderFollower" & participant == 1) |
         (condition == "Synchronous/FollowerLeader" & participant == 2))

df_EEG_desync <- df_EEG_avg %>% filter(condition %in% c("Complementary/LeaderFollower", "Complementary/FollowerLeader"))
filtered_EEG_desync <- df_EEG_sync  %>%
...
left_join(df_EEG_sync, by = c("pair", "participant"))
...
left_join(df_EEG_desync, by = c("pair", "participant"))
...
x_columns <- c('alpha_frontal', 'alpha_parietal_motor', 'alpha_parietal_parietal', 'alpha_motor_motor')  # Example y-axis variables
...
# Open a PDF device
pdf("C:/Users/nicoucke/OneDrive - UGent/Desktop/Hyperscanning 1/paper plots/Figure6_2.pdf", width = 14, height = 10)  # Adjust the width and height as needed
