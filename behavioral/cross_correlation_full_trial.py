...
def get_all_trajectories():
    distance_trajectories = pd.DataFrame(columns = ["KOP_trajectory", "PLV_trajectory", "cross_correlation", "time_differences", "pair", "success", "sync", "hierarchy", "who_first", "who_leader"])
    for Li in range(2):
        data = all_data[Li]
        if Li == 0:
            group = 'military'
    ...
    with open(r"TimeLockedCorrelationTrajectories_full_trial.pickle", "wb") as output_file:
...
with open(r"TimeLockedCorrelationTrajectories_full_trial.pickle", "rb") as input_file:
...
def plot_condition_trajectories(ax, distance_trajectories, parameters):
    ...
    # Loop through all unique values of 'pair' and select subdataframes
    unique_pairs = select_trajectories['pair'].unique()

    for pair in unique_pairs:
        sub_dataframe = select_trajectories[select_trajectories['pair'] == pair]
        
        # get a list of all lists
        list_of_lists = sub_dataframe.time_differences.to_list()

 
        merged_list = [item for sublist in list_of_lists for item in sublist]
        
        # plot a density plot
        values = np.array(merged_list)
        sns.kdeplot(data=values, color = 'green',  linewidth = 0.5, ax = ax, fill=False, alpha = 0.3)
    ...
    # plot a density plot
    values = np.array(merged_list)
    sns.kdeplot(data=values, color = 'green', linewidth = 3, ax = ax, fill=False)
    ...
    for pair in unique_pairs:
        ...
        # plot a density plot
        values = np.array(merged_list)
        sns.kdeplot(data=values, color = 'orange', linewidth = 0.5, ax = ax, fill=False, alpha = 0.)
    ...
    # plot a density plot
    values = np.array(merged_list)
    sns.kdeplot(data=values, color = 'orange', linewidth = 3, ax = ax, fill = False)
...
def plot_condition_trajectories(ax, distance_trajectories, parameters):
    ...
    colors = []
    all_trajectories = []
    for group, base_color in [('military', 'green'), ('civilian', 'orange')]:
        ...
        for pair in unique_pairs:
            ...
            traj_PLV = sub_dataframe.cross_correlation.to_list()
            trajectories = np.array([np.array(xi) for xi in traj_PLV])

            mean_trajectory = np.nanmean(trajectories, axis=0)
            x = np.linspace(-0.5, 0.5, len(mean_trajectory))

            # Calculate area under the curve
            area = np.trapz(mean_trajectory, x)
            pair_data.append((x, mean_trajectory, area))
            all_trajectories.append(mean_trajectory)

        # Sort pairs based on area under the curve
        pair_data.sort(key=lambda x: x[2])

        for i, (x, mean_trajectory, _) in enumerate(pair_data):
            verts.append(list(zip(x, mean_trajectory)))
            colors.append(mcolors.to_rgba(base_color, alpha=0.6))

    poly = PolyCollection(verts, facecolors=colors, edgecolors=colors)
    ax.add_collection3d(poly, zs=range(len(verts)), zdir='y')

    ax.set_xlim(-0.5, 0.5)
    ax.set_ylim(0, len(verts))
    
    # Correctly set z-limits
    all_trajectories = np.array(all_trajectories)
    z_min = np.min(all_trajectories)
    z_max = np.max(all_trajectories)
...
def plot_condition_trajectories(ax, distance_trajectories, parameters):
    ...
    for pair in unique_pairs:
        ...
        x = np.linspace(-0.5,0.5,len(mean_first_trajectories))

        ax.plot(x, mean_first_trajectories, color = 'green', linestyle = '-',alpha = 0.4, linewidth = 1)

    traj_PLV = select_trajectories.cross_correlation.to_list()
    first_trajectories = np.array([np.array(xi) for xi in traj_PLV])
    mean_first_trajectories = np.nanmean(first_trajectories, axis = 0)
    x = np.linspace(-0.5,0.5,len(mean_first_trajectories))
    ax.plot(x, mean_first_trajectories, color = 'green', linestyle = '-', alpha = 1, linewidth = 3)
    ...
    for pair in unique_pairs:
        ...
        ax.plot(x, mean_first_trajectories, color = 'orange', linestyle = '-', alpha = 0.6, linewidth = 1)

    traj_PLV = select_trajectories.cross_correlation.to_list()
    first_trajectories = np.array([np.array(xi) for xi in traj_PLV])
    mean_first_trajectories = np.nanmean(first_trajectories, axis = 0)
    x = np.linspace(-0.5,0.5,len(mean_first_trajectories))
    ax.plot(x, mean_first_trajectories, color = 'orange', linestyle = '-', alpha = 1, linewidth = 3)
