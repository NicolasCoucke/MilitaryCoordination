...
folder_path = r"C:\Users\nicoucke\OneDrive - UGent\Desktop\Hyperscanning 1\behavioral plots"
...
for Li in range(2):
    ...
    for pair in data:
        ...
        for condition in ['Sync_Egalitarian', 'Sync_LF', 'Sync_FL','Desync_Egalitarian', 'Desync_LF', 'Desync_FL']:
            ...
            axs1 = axs1.flatten()
            fig1.suptitle('Pair ' + str(pair.Pair) + ' ' + condition)

            # for plotting separate trajectories
            fig2, axs2 = plt.subplots(4, 5, figsize=(30, 20))
            axs2 = axs2.flatten()
            fig2.suptitle('Pair ' + str(pair.Pair) + ' ' + condition)
            ...
            for trial in pair.TrialList:
                ...
                if trial.Condition == condition:
                    ...
                    axs1[ax_i].set_aspect('equal', 'box')

                    fig1.tight_layout()

                    sc = axs1[ax_i].scatter(avg_x_coords, avg_y_coords, c=time_diffs, cmap=custom_cmap, edgecolor='none', s=70, alpha = 0.1, vmin = -0.4, vmax = 0.4)

                    axs1[ax_i].set_title('Trial ' + str(trial.TrialNumber) + '; Attempts: ' + str(attempts))

                    
                    # Add a color bar to show the scale of percentage difference
                    #plt.colorbar(sc, label='Percentage Difference')
                    ...
                    axs2[ax_i].scatter(trial.Player_1_x[startindex:], trial.Player_1_y[startindex:], alpha = 0.1, s=70, edgecolors = 'none')  
                    axs2[ax_i].scatter(trial.Player_2_x[startindex:], trial.Player_2_y[startindex:], alpha = 0.1, s=70, edgecolors = 'none') 
