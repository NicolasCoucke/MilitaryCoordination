...
for root, dirs, files in os.walk(connectivity_path):
    for name in files:
        if 'individual_power_pair' in name:
...
follower_stack = remove_outliers(follower_stack)
...
complementary_follower_stack = remove_outliers(complementary_follower_stack)
...
for contrast_idx in range(num_contrasts):
    ...
    for freq_band in range(num_freq_bands):
        ...
        if contrast_idx == 0:
            ...
            data = [leader_stack[:,freq_band,:].T, follower_stack[:,freq_band,:].T]
          
        elif contrast_idx == 2:
            data = [complementary_sync_egal_stack[:,freq_band,:].T, sync_egal_stack[:,freq_band,:].T]
           
        elif contrast_idx == 3:
            data = [complementary_leader_stack[:,freq_band,:].T, complementary_follower_stack[:,freq_band,:].T]
